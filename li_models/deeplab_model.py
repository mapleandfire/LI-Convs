"""
Modified deeplab model implementation
"""

import tensorflow.compat.v1 as tf
from deeplab.core import dense_prediction_cell
from deeplab.core import utils
import li_models.dpn_model as dpn_model
import li_models.deeplab_feature_extractor as feature_extractor
...
DPN_SCOPE_MAPPING = {
	'aspp': 'aspp', 'dense_aspp': 'denseASPP',
	'li_aspp': 'li_aspp', 'li_dense_aspp': 'li_denseASPP'}
...
def get_logits(images, model_options, image_pyramid=None):
...
def extract_features(images,
	...
	if (not model_options.aspp_with_batch_norm) or (FLAGS.dpn_variant == 'ignore'):
		# do not use dpn network
		...
		if model_options.dense_prediction_cell_config is not None:
			...
			if FLAGS.dpn_variant.startswith('li'):
				# relu will be added for dpn with lateral inhibition
				features = tf.nn.relu(features)

			if 'dense_aspp' in FLAGS.dpn_variant:
				# dense aspp variant
				concat_logits = dpn_model.build_dense_aspp(
					features,
					kernel_size=3,
					scope=DPN_SCOPE_MAPPING[FLAGS.dpn_variant],
					reduce_filters=depth,
					growth_rate=64,
					concat_projection_filters=depth,
					denseASPP_rates=tuple(FLAGS.atrous_rates),
					...
					reuse=reuse,
					is_training=is_training,
					enable_lateral_inhibition=FLAGS.dpn_variant.startswith('li'))

			else:
				# aspp variant
				name_scope = DPN_SCOPE_MAPPING['aspp'] if \
					FLAGS.li_aspp_name_scope_compatible else DPN_SCOPE_MAPPING[FLAGS.dpn_variant]
				concat_logits = dpn_model.build_aspp(
					features,
					model_options,
					kernel_size=3,
					scope=name_scope,
