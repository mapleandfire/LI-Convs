...
from dpc_utils import dense_aspp, conv_ops
...
ASPP_SCOPE = 'aspp'
KERNELWISE_ASPP_SCOPE = 'kernelwise_aspp'
...
split_separable_conv2d_kernelwise = conv_ops.split_separable_conv2d_kernelwise
...
def extract_features(images,
	...
	if not model_options.aspp_with_batch_norm:
		...
		if model_options.dense_prediction_cell_config is not None:
		...
		elif FLAGS.use_dense_aspp:
			# use dense ASPP if specified
			depth = 256
			batch_norm_params = {
			...
			concat_logits = dense_aspp.build_dense_aspp(
				features,
				concat_projection_filters=depth,  # the depth for the final concatenated output
				batch_norm_params=batch_norm_params,
				activation_fn=activation_fn,
				weight_decay=weight_decay,
				reuse=reuse,
				is_training=is_training,
				kernelwise_weights_sharing=FLAGS.kernelwise_aspp,
				kernelwise_depth_multiplier=FLAGS.kernelwise_channel_multiplier,
				kernelwise_share_batch_norm_weights=FLAGS.kernelwise_batch_norm_weights_share)
				...
				with slim.arg_scope([slim.batch_norm], **batch_norm_params):
					...
					if model_options.atrous_rates:
						...
						for i, rate in enumerate(model_options.atrous_rates, 1):
							if FLAGS.kernelwise_aspp:  # if use ASPP with kernelwise weights sharing
								# batch norm layers with the same normalizer_scope_suffix will share weights
								normalizer_scope_suffix = None \
									if FLAGS.kernelwise_batch_norm_weights_share else i
								aspp_features = split_separable_conv2d_kernelwise(
									features,
									depth,
									kernel_size=3,   # ASPP use 3*3 conv
									rate=rate,
									weight_decay=weight_decay,
									depth_multiplier=FLAGS.kernelwise_channel_multiplier,
									scope=KERNELWISE_ASPP_SCOPE,
									normalizer_fn=slim.batch_norm,
									normalizer_params=batch_norm_params,
									normalizer_scope_suffix=normalizer_scope_suffix,
									activation_fn=activation_fn,
									reuse=reuse
								)
							else:
...
def extract_backbone_features(
		...
		fine_tune_batch_norm=False,
		nas_training_hyper_parameters=None):
	"""Extracts backbone features by the particular model_variant.
